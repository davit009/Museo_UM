...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  void initState() {
    ...
    _bioController = TextEditingController(
      text: widget.currentProfileData['biografia'] ?? '',
    );
    _estadoController = TextEditingController(
      text: widget.currentProfileData['estado_actual'] ?? '',
    );
    _puestoController = TextEditingController(
      text: widget.currentProfileData['puesto_actual'] ?? '',
    );
    _empresaController = TextEditingController(
      text: widget.currentProfileData['empresa_actual'] ?? '',
    );
  }
  ...
  Future<void> _saveProfileData() async {
    ...
    final newEmpresa = widget.esEgresado
        ? _empresaController.text.trim()
        : null;

    if (ProfanityFilter.hasProfanity(newBio) ||
        ProfanityFilter.hasProfanity(newEstado) ||
        (newPuesto != null && ProfanityFilter.hasProfanity(newPuesto)) ||
        (newEmpresa != null && ProfanityFilter.hasProfanity(newEmpresa))) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Tus datos contienen lenguaje no permitido.'),
    ...
    try {
      ...
      if (mounted) {
        ...
        Navigator.pop(
          context,
          true,
        ); // Retornamos true indicando que hubo cambios
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: const [
                DropdownMenuItem(
                  value: 'ocean',
                  child: Text('Océano Profundo (Azules)'),
                ),
                DropdownMenuItem(
                  value: 'forest',
                  child: Text('Bosque Nativo (Verdes)'),
                ),
                DropdownMenuItem(
                  value: 'sunset',
                  child: Text('Ocaso del Museo (Naranjas)'),
                ),
